...
Package for distribution (runs the unit tests, then zips
`extension/` into `dist/factit-<version>.zip`):

```bash
npm run package
```

Regenerate the icons from the inline SVG in `scripts/make-icons.mjs`
(needs Chromium, like the smoke test):
...
| `action`             | toolbar button                 | Not a permission. Shows the bar / toggles the panel; `chrome.action.onClicked` provides the tab id without the `tabs` permission. |
| `icons`              | `icons/icon-*.png`             | Not a permission. Generated by `npm run icons`. |
...
scripts/make-icons.mjs           regenerates extension/icons/
scripts/package.mjs              builds dist/factit-<version>.zip
tests/
